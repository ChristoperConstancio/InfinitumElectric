    ...
    <div className="max-w-3xl mx-auto text-white mt-10 space-y-8">
      ...
      >
        ...
        <div className="border border-gray-700 rounded-xl p-4 space-y-4">
          ...
          <label className="flex items-center gap-3 text-yellow-400 font-medium cursor-pointer">
            <input
              type="checkbox"
              checked={tipoResultado === "RETRABAJO"}
              onChange={() => {
                if (tipoResultado === "RETRABAJO") {
                  // 🔄 deselecciona
                  setTipoResultado("NORMAL");
                  setRazon("");
                } else {
                  setTipoResultado("RETRABAJO");
                  setRazon("");
                }
              }}
              className="w-5 h-5 accent-yellow-400"
            />
            Retrabajo (Sí se libera)
          </label>

          {/* ❌ Rechazado */}
          <label className="flex items-center gap-3 text-red-400 font-medium cursor-pointer">
            <input
              type="checkbox"
              checked={tipoResultado === "RECHAZADO"}
              onChange={() => {
                if (tipoResultado === "RECHAZADO") {
                  // 🔄 deselecciona
                  setTipoResultado("NORMAL");
                  setRazon("");
                } else {
                  setTipoResultado("RECHAZADO");
                  setRazon("");
                }
              }}
              className="w-5 h-5 accent-red-500"
            />
            Rechazado (No se libera)
          </label>
          ...
          {tipoResultado !== "NORMAL" && (
            <textarea
              ...
              placeholder={`Razón de ${tipoResultado.toLowerCase()}...`}
